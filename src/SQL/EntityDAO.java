...
public class EntityDAO {
    ...
    public int createRecordInTable(Entity entity) {
        ...
        final String QUERY_CREATE_ON_TABLE = "INSERT INTO " + TABLE_NAME + " (" + entity.getParsedFieldsLine() + ")" + " VALUES ("
                + entity.getParsedValuesLine() + ");";
    ...
    public  boolean deleteRecordInTableByPK(Entity entity, int keyValue){
        ...
        boolean flag = false;
        final String QUERY_DELETE_ON_TABLE = "DELETE FROM " + entity.tableName() + " WHERE " + entity.primaryKey()  + " = " + keyValue;
    ...
    {
        String QUERY_SELECT_BY_ID = "SELECT * FROM " +  entity.tableName() + " WHERE " + entity.primaryKey() + " = " + id;
        try (final PreparedStatement statement = connection.prepareStatement(QUERY_SELECT_BY_ID)) {
            ...
            for (java.lang.reflect.Field parsedField : entity.getEntityClass().getDeclaredFields()) {
                if (parsedField.getName().equals(entity.primaryKey())) {
                    ...
                    parsedField.setAccessible(true);
                    try {
                        parsedField.set(entity.getEntityObject(), resultSet.getString(parsedField.getName()));
                    } catch (IllegalArgumentException | IllegalAccessException e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void updateEntity(Entity entity){
        ...
        final String QUERY_UPDATE_ON_TABLE = "UPDATE " + entity.tableName() + " SET " + preparedData + " WHERE " + entity.primaryKey()
                + " = " + entity.getPrimaryKeyValue();
        try (final PreparedStatement statement = PGConnectionPool.getInstance().getConnection().prepareStatement(QUERY_UPDATE_ON_TABLE)) {
           // statement.setInt(1, entity.getPrimaryKeyValue());
            statement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
